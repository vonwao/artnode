...
function renderLayer(layer) {
  ...
  let pixels = [];
  const offsetX = layer.offset.x, offsetY = layer.offset.y;
  ...
  rows.forEach((row, rowIndex) => {
    ...
    for (let x = 0; x < row.length; x++) {
      if (row[x] === '1') {
        const color = layer.colors ? layer.colors[rowIndex % layer.colors.length] : '#000000';
...
function generateSvgFromPixels(pixelSets, pixelSize = 20) {
  ...
  const svgLayers = pixelSets.map(pixels => pixels.map(pixel => 
    `<rect x="${pixel.x * pixelSize}" y="${pixel.y * pixelSize}" width="${pixelSize}" height="${pixelSize}" fill="${pixel.color}" />`
  ).join('')).join('');
